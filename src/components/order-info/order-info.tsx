...
export const OrderInfo: FC<IOrderInfo> = ({order, ingredients, totalCost}) => {
  ...
  return (
    ...
    <ul className={`${styles.scroll_container} mt-6`}>
      {iniqueCountedIngredients && iniqueCountedIngredients.map((el: IMainCardType, i: number) => {
              return <li className={`${styles.space_between} mt-4`} key={i}>
              <div className={styles.centered_line}>
                <div className={styles.icon_container}><img className={styles.ingredient_icon} src={el.image_mobile} alt='' /></div>
                <p className={`text text_type_main-default ml-4`}>{el.name}</p>
              </div>
              <div className={`${styles.centered_line} pl-4`}>
